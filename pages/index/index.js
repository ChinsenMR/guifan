...
Page({
  ...
  towerSwiper(name) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // console.log("list[i].mLeft ", list[i].mLeft)
    }
    // console.log(list)
    this.setData({
      swiperList: list
    })
    // console.log("list", this.data.swiperList)
  },
  // towerSwiper触摸开始
  towerStart(e) {
    // console.log("towerSwiper触摸开始", e)
    this.setData({
  ...
  //  console.log("=========",options);
    // if (options.scene) {
    //   let rid = decodeURIComponent(options.scene).split('=')[1]
    ...
    // console.log('页面状态栏高度======' + this.data.statusBarHeight)
    // console.log('获取动态高度======'+this.data.navHeight)
  ...
  onPageScroll: function(data) {
    ...
    //
    // console.log('目标高度', this.data.tabHeight);
    let flag = scrollTop >= this.data.tabHeight+80
    // console.log("输出flag",flag);
    if(flag){
      this.setData({
        isHeight:true,
