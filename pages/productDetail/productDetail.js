...
Page({
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        if (res.data.Status === "success") {
          ...
          setTimeout(function() {
            wx.navigateTo({
              url: '../../packageA/pages/CustomerService/CustomerService?onCustomer=true&id=' + id,
            })
          }, 1500)
        }
  ...
  onShow: function(e) {
    ...
    this.getIntegral(this.data.proId)
    this.getCustomer()
    // wx.setStorageSync('ReferralId', this.data.ReferralId)
    // console.log("每次进入都设置id",wx.setStorageSync('ReferralId', this.data.ReferralId))
  },
  ...
  onBuyNow: function() {
    ...
    wx.$ajax({
      ...
      //被分享的人下单了才绑定上级用户
      // bindReferralId(wx.getStorageSync("ReferralId"))
      bindReferralId(_this.data.ReferralId)
  ...
  onConNow: function() {
    warrant(data => {
      if (data) {
        ...
        if (this.data.integralList.UserExChangedNumbe >= this.data.integralList.eachMaxNumber){
          wx.showToast({
            title: '当前已达最大兑换数',
            icon:'none',
          })
