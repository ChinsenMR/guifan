      ...
      success: function(res) {
        ...
        if (res.data.Status === 'success') {
          if (linktype == 'single') {
            ...
            wx.request({
               url:getApp().data.url + '/API/VshopCollageProcess.ashx?action=VerifyIsEnableCreateCollageActivity',
               data:{
                // action:'VerifyIsEnableCreateCollageActivity',
                sessionId: wx.getStorageSync('sessionId'),
               },
               method:"POST",
               success:function(res){
                 console.log("拼团成功",res)
                 if(res.data.Status==="Success"){
                  wx.navigateTo({
