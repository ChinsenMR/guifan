  ...
  methods: {
    ...
    creatCanvas(ratio) {
      ...
      this.getImgPath(info.ImageUrl1).then((res) => {
        ctx.drawImage(res.path, 16 * ratio, 90 * ratio, 288 * ratio, 250 * ratio);
        ctx.draw(true);
      })
      // 二维码
    ...
    onShareAppMessage(res) {
      let { prDid, pageType, infoId } = this.data;
      return {
        title: this.data.shareInfo.ProductName,
        path: `/pages/home/home?prDid=${prDid}&pageType=${pageType}&rid=${infoId}`,
